...
description: |
  Singapore's top 6 merchanidise trading partners, 2019-2020
author:
  - name: Ong Chee Hong
...
The above data visualisation is created using the datasets downloaded from the sub-section of [**Merchandise Trade by Region/Market**](https://www.singstat.gov.sg/find-data/search-by-theme/trade-and-investment/merchandise-trade/latest-data) provided by the Department of Statistic, Singapore (DOS). 
...
6. Each line graph is **directly labeled** to allow user to know the identity of its line instead of the need to reference to a legend.

# 3.0 Data Visualisation

The completed data visualisation using tableau desktop can be accessed through [**here**](https://public.tableau.com/app/profile/chee.hong/viz/Singaporestop6merchandisetradingpartners2019-2020/Dashboard1). Do note to select full screen to have better visibility of the chart.
...
3. To positioned both Imports and Exports in the same pane for better comparison and visualisation. A dual axis will be performed on SUM (Export Values) and SUM (Import Values).  Both axes will thereafter be synchronised to provide an accurate representation of the data. 

![](img/Dual-axis.png)

4. Next the measured names will be removed and each line will be fixed with a label directly. The label is coloured to show distinction between the two measured values. This will also widen the chart for better viewing and also allow the user to understand the representation of each line directly.
...
6. Next, dots were outline on the line chart to better relate the value with each year/month.
...
15. As such we will make some changes to the dashboard. 1) Remove the dotted line indicator to indicate the month of interest. 2) The annotation will be change to area type instead of mark type. The finalized visualisation is as shown below.
...
1. Singapore 's import from Hong Kong for both years is **way lower than its export** as compared to other countries.
...
3. Malaysia trade with Singapore has a **steep decrease in the month of Jan 2020 to Apr 2020**. The decrease might be due to the possibility of a border closure between Singapore and Malaysia during the Covid-19 period. However, to avoid the **storytelling bug**, this possibilty will not be annotated in the chart.
...
3.	Taiwan’s trade with Singapore has a **gradual increase in the year of 2020**. Taiwan is the only country that seemingly having a gradual increase in both years of study.
