...
description: |
  Singapore's Balance of Merchandise Trade
author:
  - name: Ong Chee Hong
...
4) The label for each country only displays the net amount for 2020. It does not display any other useful information such as balance of trade, %trade surplus/trade deficit.|A suggesion is to deploy tooltip for every country displayed on the chart with useful information so that readers 1. do not need to reference to the scale to determine the value and 2. able to visualize more useful information.
...
5) The overlapped markers make it difficult for readers to visualize the real size of the marker. Although the marker was highlighted when the cursor was moved onto it, it does not allow the reader to visualize instantly.Additionally, the 45 degree reference line was covered by the markers making it difficult for readers to visualize whether import is more than export or vice versa.| **Same 3) suggestion, the markers will be filled with certain transparency and the 45 degree reference line will be thicken with darker colors.**
6) The bottom import and export scale label does not reflect well on which scale is import and export. | **All labels will be situated in the middle of the axis.**
...
4) The label for China is out of the chart which makes it unpleasant for readers to visualize.| **All markers and labels will be within the chart.**
5) As there are too many variables of interest, the huge labels make the whole chart looks cluttered.| **The proposed design will make heavily make use of tooltips with possible small labels.**
...
Next, at the first cell (**row PF122**) of the modified dataset, input the formula **PF3*$PF$132**. Lastly, drag the formularize cell to the last cell **cell TU127**
...
56. Thereafter, we will check the tooltip again. Notice that the value have change to billion. To ease the reader on reading the tooltip, we will format the tooltip by changing to $B and also change the %Surplus/Deficit to percentage.
...
112. The finalized visualisation will looks like this in Tableau Public. Access the link [here] (https://public.tableau.com/app/profile/chee.hong/viz/SingaporeMerchandiseTradeJan2011-Dec2020/Dashboard1) 
